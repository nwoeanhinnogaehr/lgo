...
struct PrunedSearchImplWrapperImpl : Impl {
    ...
    return_t on_enter(State<size> &state, minimax_t &alpha, minimax_t &beta, size_t depth, bool &terminal) {
...
template <pos_t size, typename Impl, template <pos_t, typename, typename> typename First>
struct PrunedSearchImplWrapper<size, Impl, First>
    : PrunedSearchImplWrapperImpl<size, Impl, First> { };
template <pos_t size, typename Impl, template <pos_t, typename> typename Search,
...
struct PrunedSearch {
    ...
    typename ImplWrapper::return_t search(State<size> &state) {
        return impl.search(state);
    }
};

// conjectures below
template <pos_t size, typename minimax_t, typename return_t>
struct NullPruningConjecture {
