...
TEST_CASE("alpha beta size 3", "[alphabeta]") {
    AlphaBeta<3> ab;
    State<3> s;
    REQUIRE(ab.alphabeta(s, BLACK).get_minimax() == 3);
}
TEST_CASE("alpha beta size 4", "[alphabeta]") {
    AlphaBeta<4> ab;
    State<4> s;
    REQUIRE(ab.alphabeta(s, BLACK).get_minimax() == 4);
}
...
TEST_CASE("alpha beta size 6", "[alphabeta]") {
    AlphaBeta<6> ab;
    State<6> s;
    REQUIRE(ab.alphabeta(s, BLACK).get_minimax() == 1);
}
TEST_CASE("lgo_small pv 3:1", "[alphabeta]") {
    AlphaBeta<3> ab;
    State<3> s;
    s.play(Move(BLACK, 1));
    REQUIRE(ab.alphabeta(s, WHITE).get_minimax() == 3);
}
...
TEST_CASE("lgo_small pv 4:1", "[alphabeta]") {
    ...
    State<4> s;
    s.play(Move(BLACK, 1));
    REQUIRE(ab.alphabeta(s, WHITE).get_minimax() == 4);
}
...
TEST_CASE("lgo_small pv 5:2,1,3", "[alphabeta]") {
    ...
    s.play(Move(WHITE, 1));
    s.play(Move(BLACK, 3));
    REQUIRE(ab.alphabeta(s, WHITE).get_minimax() == 0);
}
TEST_CASE("lgo_small pv 5:0,3", "[alphabeta]") {
    ...
    s.play(Move(BLACK, 0));
    s.play(Move(WHITE, 3));
    REQUIRE(ab.alphabeta(s, BLACK).get_minimax() == -5);
}
